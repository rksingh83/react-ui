...
const OTP = ({ history }) => {
  ...
  const otpHandler = () => {
    Post("/otpVerification", { emailOtp }).then((res) => {
      if (res.data.error) {
...
export const VerifyOTP = ({ history }) => {
  ...
  const otpHandler = () => {
    Post("/forgotOtpVerification", { emailOtp, email }).then((res) => {
      if (res.data.error) {
        alert(res.data.error);
      }
      if (res.data.code == 200) {
...
export const ForgotPassword = ({ history }) => {
  ...
  const sendOtpHandler = () => {
    if (email == "") toast("Enter Email");

    Post("/forgotpassword", { email }).then((res) => {
      if (res.data.error) {
        alert(res.data.error);
        if (res.data.code == 200) {
  ...
  const savePassHandler = () => {
    Post("/newPassword", { password, email }).then((res) => {
      if (res.data.error) {
        alert(res.data.error);
      }
      if (res.data.code == 200) {
